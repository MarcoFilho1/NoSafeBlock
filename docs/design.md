...
- Geometria em duas camadas, definida em `src/world/props.gd`. Estrutura carrega a silhueta e projeta sombra; decoração só acrescenta polígonos ao que a estrutura já colocou na tela, some antes (`DETAIL_RANGE`) e nunca entra no passe de sombra. O custo do detalhe fica no preenchimento perto da câmera, não no quadro inteiro. O kit tem caixa, cilindro, cone, esfera, cunha, anel e viga entre dois pontos, mais composições urbanas (janela, platibanda, caixa d'água, exaustor, antena, escada de incêndio, poste, hidrante, lixeira, tambor, cone de trânsito, banco e cerca).
- Mobiliário urbano é sempre visual: nada acrescentado por decoração tem colisão, então o bake da NavMesh e as rotas existentes não mudam.
