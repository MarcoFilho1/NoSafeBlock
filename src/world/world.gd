...
## Stretches of road between two crossings, used to lay kerbs that open at every
## junction instead of running straight through it.
const BLOCK_SPANS: Array = [[-90.0, -64.0], [-56.0, -34.0], [-26.0, -4.0], [4.0, 26.0], [34.0, 56.0], [64.0, 90.0]]
const LANES: Array = [-60.0, -30.0, 0.0, 30.0, 60.0]
...
func _ready() -> void:
	...
	for i in [-1, 1]:
		...
		# Buttresses and a coping rail keep the perimeter from reading as a ribbon.
		Props.detail(Props.box(region, Vector3(181, 0.35, 0.75), Vector3(0, 3.1, i * 90), Color("4a5a57")))
		Props.detail(Props.box(region, Vector3(0.75, 0.35, 181), Vector3(i * 90, 3.1, 0), Color("4a5a57")))
		for at in range(-84, 85, 12):
			Props.detail(Props.box(region, Vector3(0.9, 3.4, 0.8), Vector3(at, 1.7, i * 89.6), Color("30403e")))
			Props.detail(Props.box(region, Vector3(0.8, 3.4, 0.9), Vector3(i * 89.6, 1.7, at), Color("30403e")))
	build_street_dressing()
...
func build_city() -> void:
	...
	for lane in [-60, -30, 0, 30, 60]:
		...
		# Raised kerbs give the roads an edge instead of a change of paint. They
		# stop short of every crossing, the way a real kerb opens at a junction.
		for side in [-4.2, 4.2]:
			for span in BLOCK_SPANS:
				var length: float = span[1] - span[0]
				var middle: float = (span[0] + span[1]) / 2.0
				Props.detail(Props.box(region, Vector3(0.36, 0.17, length), Vector3(lane + side, 0.085, middle), Color("6d766c")))
				Props.detail(Props.box(region, Vector3(length, 0.17, 0.36), Vector3(middle, 0.086, lane + side), Color("6d766c")))
	...
	for side in [-1, 1]:
		...
		for z in [-2.6, 2.4]:
			var wheel := Props.detail(Props.cylinder(bus, 0.45, 0.3, Vector3(side * 1.2, -0.7, z), Color("1e2729"), 10))
			wheel.rotation.z = PI / 2.0
		Props.detail(Props.box(bus, Vector3(0.12, 0.45, 7.4), Vector3(side * 1.3, -0.25, 0), Color("473d34")))
	Props.detail(Props.box(bus, Vector3(1.9, 0.7, 0.08), Vector3(0, 0.6, -4.05), Color("1c292b")))
	Props.detail(Props.wedge(bus, Vector3(2.6, 0.35, 8.0), Vector3(0, 1.32, 0), Color("4a4036")))
	# The fuel canopy now stands on columns instead of floating over the forecourt.
	Props.box(region, Vector3(8, 0.4, 10), Vector3(54, 4, 15), Color("706657"))
	Props.detail(Props.box(region, Vector3(8.6, 0.5, 10.6), Vector3(54, 3.7, 15), Color("5c5346")))
	Props.detail(Props.wedge(region, Vector3(8.8, 0.7, 10.8), Vector3(54, 4.5, 15), Color("665c4d")))
	for x in [51.2, 56.8]:
		for z in [11.2, 18.8]:
			Props.detail(Props.box(region, Vector3(0.45, 3.5, 0.45), Vector3(x, 1.75, z), Color("6a6154")))
			Props.detail(Props.box(region, Vector3(0.75, 0.22, 0.75), Vector3(x, 0.11, z), Color("545c52")))
	Props.detail(Props.box(region, Vector3(9.4, 0.12, 11.4), Vector3(54, 0.06, 15), Color("5a6159")))
	for z in [12, 18]:
		...
		Props.detail(Props.wedge(region, Vector3(1.0, 0.3, 1.0), Vector3(54, 1.85, z), Color("8a6749")))
		Props.detail(Props.cylinder(region, 0.06, 1.2, Vector3(54.5, 1.5, z + 0.3), Color("3a4341"), 6))
...
func build_street_dressing() -> void:
	## Street furniture is purely visual: nothing added here carries collision, so
	## the navigation bake and every existing route are untouched.
	for lane_x in LANES:
		for lane_z in LANES:
			var corner := Vector3(lane_x + 5.6, 0, lane_z + 5.6)
			if absf(corner.x) < 13 and absf(corner.z) < 13:
				continue
			Props.lamp_post(region, corner, Color("55605c"))
			Props.detail(Props.cylinder(region, 0.55, 0.03, Vector3(lane_x - 2.2, 0.05, lane_z + 2.2), Color("39433f"), 12))
			Props.detail(Props.ring(region, 0.5, 0.58, Vector3(lane_x - 2.2, 0.06, lane_z + 2.2), Color("2c3532")))
			# Crossings sit just clear of the junction box on both approaches.
			for stripe in range(8):
				Props.detail(Props.box(region, Vector3(0.55, 0.04, 2.4), Vector3(lane_x - 3.15 + stripe * 0.9, 0.05, lane_z + 5.5), Color("919a8b")))
				Props.detail(Props.box(region, Vector3(2.4, 0.04, 0.55), Vector3(lane_x + 5.5, 0.051, lane_z - 3.15 + stripe * 0.9), Color("919a8b")))
	# Downtown is where the camera spends the match, so it carries the most props.
	for spot in [Vector3(-6.2, 0, -6.2), Vector3(6.3, 0, -6.4), Vector3(-6.4, 0, 6.3)]:
		Props.hydrant(region, spot)
	for spot in [Vector3(-7.6, 0, -5.6), Vector3(6.2, 0, 7.4), Vector3(13.0, 0, -6.2)]:
		Props.bin(region, spot, Color("4f5a52"))
	for spot in [Vector3(3.2, 0, -6.0), Vector3(3.9, 0, -5.2), Vector3(4.6, 0, -4.4)]:
		Props.traffic_cone(region, spot)
	for spot in [Vector3(-6.0, 0, 12.5), Vector3(-6.0, 0, 13.6), Vector3(6.2, 0, -12.0)]:
		Props.barrel(region, spot, Color("7a6a4b"))
	for spot in [Vector3(-6.5, 0, 9.5), Vector3(13.5, 0, 6.5)]:
		Props.bench(region, spot, Color("8a7b5c"))
	for spot in [Vector3(-11.5, 0, 5.4), Vector3(11.5, 0, -5.4)]:
		Props.lamp_post(region, spot, Color("55605c"))
