extends RefCounted
const IDS := ["walker", "runner", "spitter", "demolisher", "screamer", "volatile", "executioner", "matriarch", "aberration"]
const ROWS := [
...
static func get_definition(id: String) -> Dictionary:
	...
	return {"name": r[0], "health": r[1], "speed": r[2], "damage": r[3], "points": r[4], "color": Color(r[5]), "scale": r[6], "boss": index >= 6}
